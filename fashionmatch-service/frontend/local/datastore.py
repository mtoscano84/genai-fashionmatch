...
class Datastore():
    ...
    async def load_queries_table(self):
        ...
        """
        queries = [
            (1, 'SELECT path FROM catalog ORDER BY embedding <-> (SELECT embedding FROM image_lookup WHERE ID = $1) LIMIT 5'),
            (2, 'SELECT path FROM catalog WHERE units != 0 and price < $2 ORDER BY embedding <-> (SELECT embedding FROM image_lookup WHERE ID = $1) LIMIT 5'),
            (3, 'SELECT path FROM catalog WHERE units != 0 ORDER BY embedding <-> (SELECT embedding FROM image_lookup WHERE ID = $1) LIMIT 5'),
            (4, 'SELECT path FROM catalog WHERE price < $2 ORDER BY embedding <-> (SELECT embedding FROM image_lookup WHERE ID = $1) LIMIT 5')
        ]
